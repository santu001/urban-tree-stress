...
FIELD                                    SATELLITE
─────                                    ─────────
Arborcheck device                        Planet API  ──►  GEE Image Collection
   │  64 × .res files                       (Karuna's script, NOT in this repo)
   │                                             │
   ▼  [notebook 1, cell 1]                       ▼  cloud/shadow mask, radiometric
all_res_combined.csv                                normalisation, clip to AOI
   │                                             │
   ▼  [notebook 1, cell 3]                       ▼  zonal mean per tree polygon
...
2. **The Planet download and GEE extraction scripts are proprietary to Karuna Technology and are not in this repo.** The thesis describes their logic (Section 3.4) but the code is unavailable. This means `FraOst_Planet_Tree_Data.csv` cannot be regenerated from scratch without Karuna's toolset and a Planet API key. To process new trees you need Karuna to run their extraction, or to reimplement it in GEE. **This is the single biggest blocker for anyone continuing the work.**
...
|---|---|
| GEE script — tree polygons & shapefile export | https://code.earthengine.google.com/5c305ce62bdec2f9d301a7bdeb513b45 |
...
4. Reimplement or obtain the Planet/GEE extraction step so the pipeline is runnable end-to-end without Karuna.
